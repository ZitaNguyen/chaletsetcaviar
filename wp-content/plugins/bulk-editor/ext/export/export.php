...
final class WPBE_EXPORT extends WPBE_EXT {
    ...
    public function wpbe_ext_panel() {
        ...
        $data['download_link_xml'] = $this->get_ext_link() . '__exported_files/wpbe_exported.xml';
    ...
    public function wpbe_export_posts_count() {
        ...
        switch ($_REQUEST['format']) {
            ...
            case 'xml':
                ...
                if (!empty($active_fields)) {
                    $file_path = $this->get_ext_path() . '__exported_files/wpbe_exported.xml';
        ...
        if (!empty($_REQUEST['posts_ids'])) {
            switch ($_REQUEST['format']) {
                case 'csv':
                    ...
                    break;
                case 'xml':
                    $file = $this->get_ext_path() . '__exported_files/wpbe_exported.xml';
                    $dom = new DOMDocument("1.0", "utf-8");
                    $dom->load($file);
                    $rss = $dom->firstChild;
                    $root = $rss->firstChild;
